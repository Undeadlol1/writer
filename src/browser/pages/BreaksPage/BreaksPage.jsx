						...
						<Row>
							<Col xs={12}>
								<Form onSubmit={props.handleSubmit(props.insertProject)}>
									...
									{/* <p>This means each act starts with something big which changes heroes life in a way where is no going back.</p> */}
									{/* <p>(ex. suddenly enemy armies are appeared on the horizon)</p> */}
									<p>{t("few_tips")}:</p>
									<ul>
	...
	(dispatch, ownProps) => ({
        insertProject({beginning, middle, end}) {
			...
			function insertSucces(response) {
				ownProps.reset()
				browserHistory.push(`/project/${response.id}/steps`)
			}
		},
    })
)(ConfilctsPage))